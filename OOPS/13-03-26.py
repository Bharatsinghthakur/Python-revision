...
print(
    class_emp1.fullname()
)  # not required anything because we are calling out from instance of class then method

# here we are calling out class directory and then its method requires instance -- here its passed as self as an instance
print(
    Employee.fullname(class_emp1)
)  # will require instance as argument to known for which we are calling out
